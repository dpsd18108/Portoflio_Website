...
<?php
    ...
    $customer = $_POST['customer_name'];                  // -- 9 - Holds the customer name
    ...
    $imageFileType = strtolower(pathinfo($file_name,\FromScratch\uploads)); //-- 10d -- holds the file extension of the file (in lower case)
    
    $target_dir = "C:\xampp\htdocs\FromScratch\uploads\"; // -- 11a -- Specifies the directory where the file is going to be placed
    ...
    if(isset($_POST['submit'])) { // -- 13 -- Check if the user came to this page by pressing the submit button    
      ...
      if ($uploadOk == 0) {                // -- 18 -- Check if $uploadOk is set to 0 by an error
        ...
        if (move_uploaded_file($file_tmp_name, $target_file)) { // -- 18b -- move the file from temp to the upload folder           
            ...
            $servername = "localhost";   // -- 18c -- Connect to the MySQL database
            $username = "root";
            $password = "";
            $dbname = "test_image";
            ...
            // -- 18e -- Create a SQL INSERT statement
            $sql = "INSERT INTO customer (customer_name, picture) VALUES ('$customer', '$target_file')";      

            mysqli_query($conn, $sql); // -- 18f --Execute the SQL statement
            
            mysqli_close($conn);       // -- 18g -- Close the database connection
            
            // -- 18h -- display a confirmation message
